    ...
    def handle(self, *args, **options):
        ...
        for obj in category_block:
            ...
            parent_category, _ = models.ProductClassification.objects.get_or_create(
                name=top_category.text.upper(), highest_category=True)
            ...
            for x in other_category:
                ...
                if item:
                    act_category, _ = models.ProductClassification.objects.get_or_create(
                        name=x.text, category_id=parent_category.id)
                else:
                    lower_category, _ = models.ProductClassification.objects.get_or_create(
                        name=x.text, category_id=act_category.id)
