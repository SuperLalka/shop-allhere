...
class ApplicationForOrderingForm(forms.Form):
    ...
    order_list = forms.CharField(
        label="Состав заказа (список и количество позиций)",
        max_length=1000,
...
class ForLandlordsForm(forms.Form):
    ...
    map_link = forms.CharField(
        label="Ссылка на карту или файл с указанием месторасположения участка или объекта:",
        help_text="необязательное для заполнения поле",
        required=False
    )
    land_area = forms.CharField(
        label="Предлагаемая площадь помещения/земельного участка:",
        max_length=20
    )
    land_cadastral_number = forms.CharField(
...
class ForLeaseHoldersForm(forms.Form):
    ...
    description_company = forms.CharField(
        label="Краткое описание деятельности компании",
        max_length=500,
        help_text="Текст не более 500 символов",
        widget=forms.Textarea(attrs={'rows': 3})
    )
    presented_tm = forms.CharField(
        label="Представленные торговые марки",
        max_length=300,
        help_text="Текст не более 300 символов",
        widget=forms.Textarea(attrs={'rows': 3})
    )
    current_stores = forms.CharField(
        label="Информация о действующих магазинах сети с указанием адреса",
        max_length=300,
        help_text="Текст не более 300 символов",
        widget=forms.Textarea(attrs={'rows': 3})
    )
    ...
    interested_region = forms.CharField(
        label="Какой город/регион Вас интересует?",
        max_length=100,
        help_text="Текст не более 100 символов",
        widget=forms.Textarea(attrs={'rows': 1})
    )
    competitor_store = forms.CharField(
        label="Кого вы считаете прямым конкурентом магазина, сети?",
        max_length=100,
        help_text="Текст не более 100 символов",
        widget=forms.Textarea(attrs={'rows': 1})
    )
    ...
    required_footage = forms.CharField(
        label="Необходимый метраж помещения (минимальный, максимальный, оптимальный)",
        max_length=50
    )
